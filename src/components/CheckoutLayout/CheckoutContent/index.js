...
import { useAddress } from '~/components/MyAccount/service';
...
function CheckoutContent() {
    ...
    const handleCloseEditForm = (data) => {
        ...
        if (
            isNullOrEmpty(data.fullName) &&
            isNullOrEmpty(data.phoneNumber) &&
            isNullOrEmpty(data.address) &&
            isNullOrEmpty(data.city)
        ) {
            setEditingData(selectedAddress);
        } else {
            setEditingData(data);
        }
    };

    const handleCloseChangeForm = (data) => {
    ...
    const handleCloseAndOpenEditForm = (data) => {
        setEditingData(data);
        setChangeAddresForm(!changeAddresForm);
        setOpenEditForm(!openEditForm);
    };

    const handlePlaceOrder = (tag) => {
        var order = {
            status: 'PENDING CONFIRMATION',
            payment: tag === 'COD' ? 'COD' : 'PayPal',
            add: selectedAddress,
            product: cartItems,
            total: calculateTotalPrice(canculateSubTotal(), shippingMethods[selectedMethod].price),
            shipping: shippingMethods[selectedMethod].name,
        };

        const formData = new FormData();
        formData.append('status', order.status);
        formData.append('paymentStatus', order.payment);
        formData.append(
            'address',
            `${selectedAddress.fullName}, ${selectedAddress.address}, ${selectedAddress.city}, (${selectedAddress.phoneNumber})`,
        );
        formData.append('total', order.total);
        formData.append('shippingMethod', order.shipping);
        const itemIds = order.product.map((item) => item.cartItemId).join(',');
                                ...
                                <div>
                                    <section>
                                        ...
                                        {changeAddresForm && (
                                            <ChangeAddress
                                                ...
                                                onOpenEditForm={handleCloseAndOpenEditForm}
                                            ></ChangeAddress>
                                        )}
                                        <div className={cx('loading_style-wrapper')}>
                                                    ...
                                                    <div key={item.index}>
                                                        ...
                                                        >
                                                            ...
                                                            <div
                                                                ...
                                                                className={cx('radio_button-style')}
                                                            ></div>
                                                        </label>
                                                    </div>
                                                ))}
                                    ...
                                    <section>
                                                ...
                                                {PaymentMethods.map((item) => (
                                                    <div key={item.id}>
                                                        <div
                                                            ...
                                                            className={cx('accordion_item_wrapper')}
                                                        >
                                                            <div
                                                                onClick={() => handleChoosePaymentMethod(item.id)}
                                                                className={cx('header_button')}
                                                            >
                                        ...
                                        <p className={cx('primary_payment_des')}>
                                            <span>
                                                By placing your order, you agree to the Terms and Conditions and Privacy
                                                Policy. Please note, we use your personal information to process,
                                                deliver and support your order. We may also use your personal
                                                information to email you in order to gather feedback on our services. To
                                                find out how to control your personal data, please see our Privacy
                                                Policy.
                                            </span>
                                        </p>
                                            ...
                                            <div className={cx('payment_button_container')}>
                                                <button>
                                                    <span
                                                        onClick={() =>
                                                            handlePlaceOrder(
                                                                selectedPaymentMethod === 1 ? 'COD' : 'PayPal',
                                                            )
                                                        }
                                                    >
                                                        {selectedPaymentMethod === 1 ? 'Place order' : 'PayPal'}
                                                    </span>
                                                </button>
                                                <span>{`Total: $${calculateTotalPrice(
                                                    canculateSubTotal(),
                                                    shippingMethods[selectedMethod].price,
                                                )}`}</span>
                                            </div>
                                        </div>
                                    </section>
                                    ...
                                    <div className={cx('summary_wrapper')}>
                                        ...
                                        <div className={cx('sidebar_line')}>
                                            ...
                                            <span className={cx('sidebar_line-text')}>
                                                <span>{`$${canculateSubTotal()}`}</span>
                                            </span>
                                        </div>
                                        <div className={cx('sidebar_line')}>
                                            <span className={cx('sidebar_line-text')}>
                                                {shippingMethods[selectedMethod].name}
                                            </span>
                                            <span className={cx('sidebar_line-text')}>
                                                {shippingMethods[selectedMethod].price === 0
                                                    ? 'Free'
                                                    : `$${shippingMethods[selectedMethod].price}`}
                                            </span>
                                        </div>
                                        <div className={cx('sidebar_line')}>
                                            <span
                                                ...
                                                className={cx('sidebar_line-text')}
                                            >
                                                Order Total
                                            </span>
                                            <span
                                                            ...
                                                            <div className={cx('line_item-section')}>
                                                                ...
                                                                {cartItems &&
                                                                    cartItems.map((item) => (
                                                                        <div
                                                                            key={item.cartItemId}
                                                                            className={cx('line_item-section-content')}
                                                                        >
                                                                            <div
                                                                                className={cx('product_row_container')}
                                                                            >
                                                                                <div className={cx('lineitem_detail')}>
                                                                                    ...
                                                                                    >
                                                                                        <img
                                                                                            src={`http://localhost:8080/img/${item.product.images[0]}`}
                                                                                        ></img>
                                                                                    </div>
                                                                                    <div
                                                                                        className={cx(
                                                                                            'lineitem_detail-container',
                                                                                        )}
                                                                                    >
                                                                                        <div
                                                                                            className={cx(
